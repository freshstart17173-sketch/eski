...
`gaps.md` gap-analysis was fully built and has been removed; its S/B items are
folded in below.) Brand art lives separately in
[`brand-assets-todo.md`](brand-assets-todo.md); project state in
[`../../memory.md`](../../memory.md). Check against CANON before implementing.

> **The #1–#61 edit list and the S/B gap backlog are done (2026-08-21/22).**
...
## Determinism system (done 2026-08-22)

The gallery is now self-verifying — see
[`gallery-determinism-plan.md`](gallery-determinism-plan.md) (rationale),
[`STATES.md`](STATES.md) (the review checklist), and [`verify.mjs`](verify.mjs)
(the gate: `node verify.mjs`). Every state is a URL (`?app=1#screen/state`,
`#dialog/id`, `&w=`, `&theme=`); the §⑥ dialog catalog is **generated by cloning
the real nodes**; a real-media **asset system** lives in [`assets/`](assets/).
Web-only scaling (mobile deferred, CANON §C.2).

## Next tasks (not started)

- [x] **Exploded-view catalog panels** (2026-08-22) — **superseded by generation.**
...
- [ ] **Mobile gallery — DEFERRED post-beta (2026-08-22).** The beta is web-only
  (CANON §C.2): desktop three-pane, scaling 1024↔1440 (the gallery's `&w=` knob +
  `verify.mjs` check both ends). When mobile returns, it's a **separate**
  `gallery-mobile.html` against the (retained, dormant) CANON §C.2 contract —
  three-pane → one pane + bottom tabs, channel drawer, members sheet, details
  bottom-sheet. Not a responsive pass on the desktop file.
