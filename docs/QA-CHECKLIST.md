...
- [ ] `[P23 folder tags]` As a server admin (or in My files), a folder card shows its first ~3 tags (grey for a plain tag, coloured for a typed tag like `bpm:140`); a `+N` chip and the folder's right-click **Properties** open a popover listing ALL tags with an add box + remove ✕. Add/remove a tag → it persists after reload; a non-admin member can VIEW a server folder's tags but the add box/✕ are absent. List view shows a few tags after the folder name; small/icon view shows none. Clicking a folder tag searches the library for it.
