...
- [ ] **(P14 view densities)** The view switcher offers **Large · Small icons · List** (Windows-Explorer densities). **Large** shows big content thumbnails (real image/video files show their thumbnail; other kinds show a kind icon) with filename + uploader; **Small icons** is a dense `[icon · name]` grid; **List** is a Details table with **Name · Type · Size · Uploader · Added** columns. In every density: single-click selects, double-click opens, the marquee/⌘A/bulk-bar/drag-to-make-folder all work, and the chosen density persists in the URL (`?view=small|list`) across reload.
- [ ] Search files → searches the whole tree, not just the current folder.
- [ ] **(P24 / real search)** Type a term → results include files whose **tags** contain it, not just the filename (**B19**). Type **`bpm:120`** → the search field **tints** (modifier recognised) and results narrow to files with that exact typed tag. Type **`hastag:key`** → only files carrying a `key:…` tag. Type **`sortby:bpm_desc`** → results order by their bpm value, high→low. With a big library the search returns fast (it runs server-side, `search_files`), shows a brief "Searching…", and a **"Load more"** appears when there are more than one page of results. The old **"Tag type" filter dropdown is gone** (use `hastag:` instead).
- [ ] `[P23 folder tags]` As a server admin (or in My files), a folder card shows its first ~3 tags (grey for a plain tag, coloured for a typed tag like `bpm:140`); a `+N` chip and the folder's right-click **Properties** open a popover listing ALL tags with an add box + remove ✕. Add/remove a tag → it persists after reload; a non-admin member can VIEW a server folder's tags but the add box/✕ are absent. List view shows a few tags after the folder name; small/icon view shows none. Clicking a folder tag searches the library for it. **Upload:** dropping/picking a FOLDER shows a "Folder tags" section with a tag row per subfolder → after upload, each subfolder carries its own tags (its files do NOT inherit them); ticking "Flatten folders" drops the folder tags.
- [ ] **(B35 / substring)** Type a **partial** term — a fragment mid-name, not a whole word (e.g. `wil` for a "Willow" file, `158bpm`, `b.d.y`) → the file still appears. Before the fix these returned nothing (filename matched whole words only); now the filename is matched as a substring. Whole words and tag matches still work as before.
- [ ] **(P24)** Search results respect visibility — you never see another server's or another person's private files in your results (the RPC is fenced by RLS).
...
- [ ] **(B13 — menu permission gate)** As a NON-admin member, open the ⋯ menu (and the details-pane menu) on **another member's** server file → you see only **Star · Save to my files · Share… · Copy link** (no Change visibility / Rename / Move to… / Hide / Delete). On **your own** file, or as a **server admin** on any file, all of those write items are present. (Logic-verified in demo; demo runs as an admin so it always shows the full set.)
