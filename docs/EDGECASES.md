...
| P7 | **Slider raised mid-cycle** (e.g. server 80 → 200 GB with 15 of 30 days left) | "Billed on the level you hold / no proration" (§D.2.4) didn't say *when* the higher price hits, or how to stop park-and-drop (raise → dump a batch → drop before renewal → pay nothing extra). | **RESOLVED — prorate up, not down.** Slide up: capacity unlocks now; Stripe charges the **prorated difference** for the days remaining ((new − old)/mo × days-left ÷ cycle) — 80 GB $2.40 → 200 GB $5.88 = **~$1.74 now**, then $5.88/renewal. Slide down: **no refund**, lower level starts next renewal, over-level bytes go read-only (never deleted). The only proration avoided is refund/credit math on downgrades. Paying the difference *is* the consent, so no surprise charge; it closes the loophole. (Stripe default proration; §D.2 principle 4.) |
