-- eski schema · 36 · B35 fix — file/server search must match filename SUBSTRINGS
--
-- P24's search_files() matched the filename only through full-text (works.search_tsv), i.e. whole
-- lexemes: "willow" found "Willow Smith …" but "wil" found NOTHING, and "81.5bpm" / "b.d.y" / any
-- mid-word fragment found nothing either. A file browser is searched by typing a *fragment* of a
-- name, so from the owner's seat "server search doesn't work at all" (B35). The RPC, its args, and
-- the client wiring were all correct — the matcher was just too strict.
--
-- Fix: the free-text clause now also matches `title ILIKE '%term%'` (a plain substring), alongside
-- the existing filename FTS (kept for stemming/ranking of real words) and the B19 tag-contains. So
-- "wil" → "Willow …", "158bpm" → the file whose name carries it, "idyll" → "… Idyll.mp3". A trigram
-- GIN on works.title keeps that ILIKE fast at gigabyte scale (same device pg_trgm already powers for
-- content_tags in schema-35) — this is the filename half of K12's "search stays fast" goal.
--
-- Still SECURITY INVOKER: works_read RLS (= can_read_work) fences visibility; the substring is only
-- another way to MATCH within the caller's already-visible set, never a way to widen it. Only the
-- free-text predicate changed; scope, facets, sort, pagination, grants are byte-identical to p26.

-- pg_trgm already enabled by schema-35; the index it powers here is new.
create extension if not exists pg_trgm with schema extensions;

-- Trigram GIN on the filename: supports ILIKE '%fragment%' without a seq scan as libraries grow.
-- gin_trgm_ops handles case-insensitivity itself, so index the raw column (not lower(title)).
...
create or replace function search_files(
  ...
  p_text     text        default null,        -- free text: filename FTS + filename substring + tag contains
  ...
  base as (
    select w.id, w.title, w.file_ext, w.kind, w.blob_sha, w.bytes, w.created_at, w.author_id, w.hidden
     ...
     where w.deleted_at is null
       ...
       -- free text (B35): filename full-text (words/stemming) OR filename substring (fragments —
       -- "wil" → "Willow …") OR any tag contains the term (B19). qtext null = no text filter.
       and (
